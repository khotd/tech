...
class ProfileCreation(StatesGroup):
    ...
    waiting_for_preferred_city = State()
        ...
        try:
            ...
            if method == "PUT":
                async with session.put(f"{API_BASE}{endpoint}", json=json_data, timeout=timeout) as resp:
                    return await resp.json()
            if method == "DELETE":
                async with session.delete(f"{API_BASE}{endpoint}", timeout=timeout) as resp:
...
async def cmd_start(message: types.Message):
    ...
    if message.text:
        ...
        if len(parts) > 1:
            invite_code = parts[1].strip().upper() or None
    result = await api_request(
        "POST",
        "/user/register",
        {"telegram_id": telegram_id, "username": username, "invite_code": invite_code},
    )
    if "error" in result:
        await message.answer("⚠️ Не удалось подключиться к backend. Проверьте, что API запущен.")
        return
    user_states[telegram_id] = {"user_id": result["user_id"], "invite_code": invite_code}
    ref = result.get("referral_code") or ""
    extra = ""
    if ref:
        me = await bot.get_me()
        extra = f"\n\n🎁 Твоя реферальная ссылка:\nhttps://t.me/{me.username}?start={ref}"
    await message.answer(
        "Привет! Это мини-дайтинг в стиле Дайвинчика.\n"
        "Сначала заполни анкету, потом листай выдачу свайп-кнопками." + extra,
...
async def process_city(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "В каком городе ищешь пару? (напиши название или «любой» если не важно)"
    )
    await state.set_state(ProfileCreation.waiting_for_preferred_city)


@dp.message(ProfileCreation.waiting_for_preferred_city)
async def process_preferred_city(message: types.Message, state: FSMContext):
    raw = (message.text or "").strip()
    preferred_city = "" if raw.lower() in ("любой", "any", "не важно", "-") else raw
...
async def process_bio(message: types.Message, state: FSMContext):
    ...
    await message.answer("Интересы через запятую (например: музыка, спорт, путешествия):")
    await state.set_state(ProfileCreation.waiting_for_interests)


@dp.message(ProfileCreation.waiting_for_interests)
async def process_interests(message: types.Message, state: FSMContext):
    interests = (message.text or "").strip()
    if len(interests) < 3:
        await message.answer("Добавь хотя бы пару интересов через запятую.")
        return
    ...
    result = await api_request(
        ...
        "/profile/create",
        {
            ...
            "city": payload["city"],
            "preferred_city": payload.get("preferred_city") or "",
            "bio": payload["bio"],
...
async def show_my_profile(message: types.Message, state: FSMContext):
    ...
    text = (
        ...
        f"👤 {profile.get('name', '-')}\n"
        f"🎂 {profile.get('age', '-')}\n"
        f"🏙 Ищу в: {profile.get('preferred_city') or 'любой'}\n"
        f"🎯 Интересы: {profile.get('interests', '-')[:80]}\n"
        f"📝 {profile.get('bio', '-')}\n"
...
async def recreate_profile(cb: types.CallbackQuery, state: FSMContext):
    ...
    if user_id:
        res = await api_request("DELETE", f"/profile/{user_id}")
        if "error" in res:
            logger.warning("recreate delete profile: %s", res)
    await state.clear()
...
async def process_interaction(cb: types.CallbackQuery):
    ...
    if result.get("match") and result.get("matched_profile"):
        ...
        partner_tid = match.get("telegram_id")
        if partner_tid and partner_tid != telegram_id:
            liker = cb.from_user
            my_link = f"https://t.me/{liker.username}" if liker.username else "(username скрыт — ответь через бота)"
            my_name = liker.full_name or liker.username or "Пользователь"
            uid = user_states.get(telegram_id, {}).get("user_id")
            extra = ""
            if uid:
                myp = await api_request("GET", f"/profile/{uid}")
                if "detail" not in myp:
                    extra = (
                        f"📍 {myp.get('city') or '—'}\n"
                        f"📝 {(myp.get('bio') or '')[:120]}\n"
                    )
            try:
                ...
                logger.warning("Не удалось уведомить партнёра о мэтче %s: %s", partner_tid, exc)
